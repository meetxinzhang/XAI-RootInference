...
class AGFPropSimple(AGFProp):
    def AGF(self, cam=None, grad_outputs=None, **kwargs):
        def backward(cam):
            ...
            S = safe_divide(cam, Z)
            # root = rel_sup_root_act(self.X, cam, step=50, func=self.forward)
            # signal = self.X - root
            # z = self.forward(signal) * cam.ne(0).type(cam.type())
            ...
            if not torch.is_tensor(self.X):
                ...
                # result = piece_dtd_act(x=signal, z=z, under_R=z, R=cam, root_zero=root, func=self.forward, step=20)
...
class Linear(nn.Linear, AGFProp):
    def AGF(self, cam=None, grad_outputs=None, **kwargs):
        if grad_outputs is None:
            # ours ------------------------------------------------------??????????
            # if cam is not None:
            #     root = rel_sup_root_linear_v2(self.X, cam, step=20, func=self.forward)
            #     signal = self.X - root
            #     Y = self.forward(signal)
            # else:
            #     signal = self.X.clone()
            ...
            # ours ----------------------------------------------------- ???????????
            # grad_out = torch.autograd.grad(Y, self.X, tgt)
            # result = piece_dtd_linear_v2(x=signal, z=Y, S=tgt, root_zero=root, func=self.forward, step=20)
        else:
            # Compute - C
            xabs = self.X.abs()
            wabs = self.weight.abs()
            # ours -----------------------------------------------------
            root = rel_sup_root_linear_v2(xabs, cam, step=50, func=self.forward)
            signal = xabs - root
            Zabs = F.linear(signal, wabs) * cam.ne(0).type(cam.type())
            # Gradients stream
            ...
            # signal = self.X - root
            # Y = self.forward(signal)
            # grad_out = torch.autograd.grad(Y, signal, S)
...
class Conv2d(nn.Conv2d, AGFProp):
    ...
    def AGF(self, cam=None, grad_outputs=None, **kwargs):
        ...
        # ours -----------------------------------------------------
        root = rel_sup_root_cnn_v2(self.X, cam, step=50, func=self.forward)
        signal = self.X - root
        zp = F.conv2d(signal, pw, stride=self.stride, padding=self.padding)
        zn = F.conv2d(signal, nw, stride=self.stride, padding=self.padding)
        _R1 = cam * torch.abs(zp) / (torch.abs(zp) + torch.abs(zn))
        _R2 = cam * torch.abs(zn) / (torch.abs(zp) + torch.abs(zn))
        cp = piece_dtd_conv(signal, pw, self.stride, self.padding, z=zp, under_R=zp, R=_R1, root_zero=root, step=50)
        cn = piece_dtd_conv(signal, nw, self.stride, self.padding, z=zn, under_R=zn, R=_R2, root_zero=root, step=50)
        R = cp + cn
        # z = self.forward(signal)
        # R = piece_dtd_conv_v2(x=signal, z=z, under_R=z, R=cam, root_zero=root, func=self.forward, step=20)
        M = F.relu(R.mean(dim=1, keepdim=True).expand_as(self.X))
        ...
        # C = xabs * grad[0]
        # ours -----------------------------------------------------
        root = rel_sup_root_cnn(xabs, cam, step=50, the_layer=[wabs, self.stride, self.padding])
        signal = self.X - root
        z = self.forward(signal)
        C = piece_dtd_conv(x=signal, w=wabs, z=z, under_R=z, R=cam, root_zero=root, step=50,
        ...
        signal = self.X - root
        z = self.forward(signal)
        A = piece_dtd_conv(x=signal, w=wabs, z=z, under_R=z, R=cam, root_zero=root, step=50,
                           stride=self.stride, padding=self.padding)
