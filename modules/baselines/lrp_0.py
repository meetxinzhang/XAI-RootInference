...
class DTD(nn.Module):
    ...
    def backprop_dense(self, activation, module, R):
        ...
        Z = F.linear(activation, W)
        # S = R / Z
        # C = torch.mm(S, W)  # R/Z * grad
        # C = torch.autograd.grad(Z, activation, S)[0]
        # R = activation * C
        # !!!!!------------ Ablation experiment for the proposed root -------------------!!!!!
        R = dtd_linear_piecewise_sample(x=activation, w=W, z=Z, under_R=Z, R=R, root_zero=torch.zeros_like(activation),
                                        func=F.linear)
    ...
    def backprop_conv(self, activation, module, R, layer_idx=9):
        ...
        # R = activation * C
        # !!!!!------------ Ablation experiment for the proposed root -------------------!!!!!
        R = dtd_conv_piecewise_sample(activation, W, stride, padding, z=Z, under_R=Z, R=R,
                                      root_zero=torch.zeros_like(activation))
