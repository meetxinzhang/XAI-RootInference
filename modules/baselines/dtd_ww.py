...
class DTD(nn.Module):
    ...
    def backprop_dense(self, activation, module, R):
        ...
        # w**2
        w_squared = W * W  # [1000, 512]
        Z = torch.sum(w_squared, dim=-1) + 1e-9  # [1000,]
        S = R / Z
        R = torch.mm(torch.transpose(w_squared, 0, 1), torch.transpose(S, 0, 1))
    ...
    def backprop_conv(self, activation, module, R, layer_idx=9):
        ...
        # w**2
        w_squared = W * W  # [1000, 512]
        one = torch.ones_like(activation, dtype=activation.dtype, requires_grad=True)
        Z = F.conv2d(one, w_squared, stride=stride, padding=padding) + 1e-9
        S = R / Z
        R = F.conv_transpose2d(S, w_squared, stride=stride, padding=padding, output_padding=output_padding)
        # Z = torch.sum(w_squared, dim=-1) + 1e-9  # [1000,]
        # S = R / Z
        # R = torch.mm(torch.transpose(w_squared, 0, 1), torch.transpose(S, 0, 1))
        # R = torch.transpose(R, 0, 1)
