...
def infer_linear(x, R, step=50, weight=None, z=None, samples=5):
    lr = 1
    signal_new = torch.zeros_like(x).requires_grad_(True)
    # signal_new = x.clone().detach().requires_grad_(True)
    # x_new = torch.nn.Parameter(x_new, requires_grad=True)
    R_ = R
    _root = None
    path = []
    piece_step = step // samples
    ...
    for i in range(step):
        ...
        grad_infer = torch.autograd.grad(outputs=loss, inputs=signal_new, grad_outputs=torch.ones_like(loss))[0]
        grad_infer = torch.clamp(grad_infer, max=1)
        delta = lr * grad_infer

        signal_new = signal_new - delta
        _root = x - signal_new
        path.append(_root.detach())
...
def infer_act(x, R, z, step=50, func=None):
    # signal_new = x.clone().detach().requires_grad_(True)
    # gradient-based gamma
...
def infer_cnn(x, R, step=50, the_layer=None, z=None):
    lr = 1
    signal_new = x.clone().detach().requires_grad_(True)
    # grad_x = torch.autograd.grad(outputs=z, inputs=x, grad_outputs=torch.ones_like(z), retain_graph=True)[0]
